...
MODE="${1:-}"
[[ "$MODE" == preflight || "$MODE" == prepare || "$MODE" == partial-preflight || "$MODE" == recover-partial || "$MODE" == reject-incomplete-bundle || "$MODE" == resume-prepare || "$MODE" == resume-after-venv-build || "$MODE" == resume-after-control-stage || "$MODE" == finalize-schema-acceptance || "$MODE" == advance-prepared-control || "$MODE" == verify-prepared || "$MODE" == install-unit ]] || {
  echo "usage: $0 preflight|prepare|partial-preflight|recover-partial|reject-incomplete-bundle|resume-prepare|resume-after-venv-build|resume-after-control-stage|finalize-schema-acceptance|advance-prepared-control|verify-prepared|install-unit [arguments]" >&2
  exit 2
}
shift
...
prepare_release_and_database() {
  ...
  /usr/bin/chmod g-s "$app_stage" "$control_stage"
  ...
  local build_archive="/opt/tu1nz_repos/backups/m4-23-input/adult-publishing-core-$APPLICATION_SHA-source.tar.gz"
  [[ ! -e "$build_archive" && ! -L "$build_archive" ]] || fail "application source archive target already exists"
  git_read "$app_target" archive --format=tar.gz --output="$build_archive" HEAD
  /usr/bin/chown root:root "$build_archive"
  /usr/bin/chmod 0600 "$build_archive"
  "$venv_target/bin/python" -m pip install --no-deps --no-build-isolation "$build_archive"
  /usr/bin/chown -R root:"$RUNTIME_GROUP" "$venv_target"
  /usr/bin/chmod 0750 "$venv_target"
  /usr/bin/chmod g-s "$venv_target"
...
verify_built_partial() {
  local app_target="$RELEASE_ROOT/application/$APPLICATION_SHA"
  local control_target="$RELEASE_ROOT/control/$BUILT_PARTIAL_CONTROL_SHA"
  local venv_target="$RELEASE_ROOT/venv/$APPLICATION_SHA"
  local expected_ignored
  expected_ignored=$'Would remove build/\nWould remove src/tu1nz_adult_publishing_core.egg-info/'
  verify_common_inputs
  verify_application_bundle
  [[ "$(sha256 "$INSTALLED_BACKUP")" == "$(sha256 "$CONTROL_REPOSITORY/scripts/tu1nz_encrypted_backup.sh")" ]] || fail "commercial-aware backup script is not installed"
  /usr/bin/getent passwd "$RUNTIME_USER" >/dev/null || fail "commercial operating-system user is absent"
  ...
  [[ "$(postgres --dbname=postgres --command="SELECT string_agg(rolname || ':' || rolcanlogin, ',' ORDER BY rolname) FROM pg_roles WHERE rolname IN ('$MIGRATOR_ROLE','$RUNTIME_ROLE')")" == "$MIGRATOR_ROLE:false,$RUNTIME_ROLE:true" ]] || fail "commercial PostgreSQL role boundary mismatch"
  [[ "$(postgres --dbname=postgres --command="SELECT pg_get_userbyid(datdba) FROM pg_database WHERE datname='$DATABASE'")" == "$MIGRATOR_ROLE" ]] || fail "commercial PostgreSQL database boundary mismatch"
  [[ "$(postgres --dbname="$DATABASE" --command="SELECT count(*) FROM pg_tables WHERE schemaname='public'")" == 0 ]] || fail "built partial commercial database is not empty"

  [[ ! -L "$app_target" && -d "$app_target/.git" ]] || fail "built partial application release is unsafe"
  [[ "$(git_read "$app_target" rev-parse HEAD)" == "$APPLICATION_SHA" ]] || fail "built partial application SHA mismatch"
  [[ "$(git_read "$app_target" rev-parse 'HEAD^{tree}')" == "$EXPECTED_APPLICATION_TREE" ]] || fail "built partial application tree mismatch"
  [[ -z "$(git_read "$app_target" status --porcelain=v1)" ]] || fail "built partial application tracked state is dirty"
  [[ "$(git_read "$app_target" clean -ndx)" == "$expected_ignored" ]] || fail "built partial application ignored-output boundary mismatch"
  git_read "$app_target" fsck --full >/dev/null
  [[ "$(stat -c '%a:%U:%G' "$app_target")" == "2750:root:$RUNTIME_GROUP" ]] || fail "built partial application metadata mismatch"

  verify_clean_release "$control_target" "$BUILT_PARTIAL_CONTROL_SHA" "$BUILT_PARTIAL_CONTROL_TREE"
  [[ "$(stat -c '%a:%U:%G' "$control_target")" == "2750:root:$RUNTIME_GROUP" ]] || fail "built partial Control metadata mismatch"
  [[ ! -L "$venv_target" && -d "$venv_target" && "$(stat -c '%a:%U:%G' "$venv_target")" == "2750:root:$RUNTIME_GROUP" ]] || fail "built partial venv metadata mismatch"
  [[ "$(PYTHONDONTWRITEBYTECODE=1 "$venv_target/bin/python" -c "import importlib.metadata,psycopg,tu1nz_sandbox;print(psycopg.__version__+'|'+importlib.metadata.version('tu1nz-adult-publishing-core'))")" == "3.3.4|0.1.0" ]] || fail "built partial venv contract mismatch"

  for parent in "$RELEASE_ROOT" "$RELEASE_ROOT/application" "$RELEASE_ROOT/control" "$RELEASE_ROOT/venv"; do
    [[ ! -L "$parent" && -d "$parent" && "$(stat -c '%a:%U:%G' "$parent")" == "2750:root:$RUNTIME_GROUP" ]] || fail "built partial release parent mismatch: $parent"
  done
  for path in "$RELEASE_ROOT/application-current" "$RELEASE_ROOT/control-current" "$RELEASE_ROOT/venv-current" "$CONFIG_ROOT" "$STATE_ROOT" "$INSTALLED_UNIT"; do
    [[ ! -e "$path" && ! -L "$path" ]] || fail "built partial target must be absent: $path"
  done
  /bin/systemctl is-active --quiet tu1nz-adult-publishing-s1.service || fail "STAGING-S1 is not active"
  ! /bin/systemctl is-active --quiet tu1nz_encrypted_backup.service || fail "backup service is active during built-partial recovery"
  /bin/systemctl is-active --quiet tu1nz_encrypted_backup.timer || fail "backup timer is not active before built-partial recovery"
  ! /bin/systemctl is-active --quiet "$UNIT" || fail "commercial unit is active"
  echo "M4_23_BUILT_PARTIAL_BOUNDARY_OK"
}

resume_after_venv_build() {
  local app_target="$RELEASE_ROOT/application/$APPLICATION_SHA"
  local control_target="$RELEASE_ROOT/control/$CONTROL_SHA"
  ...
  [[ ! -e "$evidence_target" && ! -L "$evidence_target" ]] || fail "immutable build-output evidence target already exists"
  trap resume_backup_timer EXIT
  /bin/systemctl stop tu1nz_encrypted_backup.timer
  BACKUP_TIMER_PAUSED=1

  /usr/bin/install -d -o root -g root -m 0700 "$evidence_target"
  /usr/bin/mv -- "$app_target/build" "$evidence_target/build"
  /usr/bin/mv -- "$app_target/src/tu1nz_adult_publishing_core.egg-info" "$evidence_target/tu1nz_adult_publishing_core.egg-info"
  /usr/bin/chown -R root:root "$evidence_target"
  /usr/bin/chmod 0700 "$evidence_target"
  /usr/bin/chmod 0750 "$app_target" "$old_control_target" "$venv_target"
  /usr/bin/chmod g-s "$app_target" "$old_control_target" "$venv_target"
  verify_clean_release "$app_target" "$APPLICATION_SHA" "$EXPECTED_APPLICATION_TREE"
...
verify_prepared_control_boundary() {
  local app_target="$RELEASE_ROOT/application/$APPLICATION_SHA"
  local control_target="$RELEASE_ROOT/control/$PREPARED_CONTROL_SHA"
  local venv_target="$RELEASE_ROOT/venv/$APPLICATION_SHA"
  local evidence_root="/opt/tu1nz_repos/backups/m4-23-commercial-s0-stopped-installation/20260828T15-39-59Z"
  ...
  [[ "$(postgres --dbname=postgres --command="SELECT string_agg(rolname || ':' || rolcanlogin, ',' ORDER BY rolname) FROM pg_roles WHERE rolname IN ('$MIGRATOR_ROLE','$RUNTIME_ROLE')")" == "$MIGRATOR_ROLE:false,$RUNTIME_ROLE:true" ]] || fail "commercial PostgreSQL role boundary mismatch"
  [[ "$(postgres --dbname=postgres --command="SELECT pg_get_userbyid(datdba) FROM pg_database WHERE datname='$DATABASE'")" == "$MIGRATOR_ROLE" ]] || fail "commercial PostgreSQL database boundary mismatch"
  [[ "$(postgres --dbname="$DATABASE" --command="SELECT count(*) FROM pg_tables WHERE schemaname='public'")" == 39 ]] || fail "prepared Control boundary table count mismatch"
  [[ "$(postgres --dbname="$DATABASE" --command="SELECT count(*) FROM pg_proc WHERE proname LIKE 'tu1nz_%'")" == 21 ]] || fail "prepared Control boundary function count mismatch"
  [[ "$(postgres --dbname="$DATABASE" --command="SELECT (SELECT count(*) FROM creators) || '|' || (SELECT count(*) FROM policy_versions) || '|' || (SELECT count(*) FROM country_policy_rules) || '|' || (SELECT count(*) FROM platform_policy_rules) || '|' || (SELECT count(*) FROM integration_accounts) || '|' || (SELECT count(*) FROM publication_destinations)")" == "1|1|1|3|3|3" ]] || fail "prepared Control boundary synthetic bootstrap count mismatch"
  ...
  for parent in "$RELEASE_ROOT" "$RELEASE_ROOT/application" "$RELEASE_ROOT/control" "$RELEASE_ROOT/venv"; do
    [[ ! -L "$parent" && -d "$parent" && "$(stat -c '%a:%U:%G' "$parent")" == "2750:root:$RUNTIME_GROUP" ]] || fail "prepared Control release parent mismatch: $parent"
  done
  [[ "$(PYTHONDONTWRITEBYTECODE=1 "$venv_target/bin/python" -c "import importlib.metadata,psycopg,tu1nz_sandbox;print(psycopg.__version__+'|'+importlib.metadata.version('tu1nz-adult-publishing-core'))")" == "3.3.4|0.1.0" ]] || fail "prepared Control venv contract mismatch"
  [[ ! -L "$evidence_target" && -d "$evidence_target" && "$(stat -c '%a:%U:%G' "$evidence_target")" == "700:root:root" ]] || fail "prepared Control build-output evidence mismatch"

  [[ "$(readlink "$RELEASE_ROOT/application-current")" == "application/$APPLICATION_SHA" ]] || fail "prepared Control application link mismatch"
  [[ "$(readlink "$RELEASE_ROOT/control-current")" == "control/$PREPARED_CONTROL_SHA" ]] || fail "prepared Control active link mismatch"
  [[ "$(readlink "$RELEASE_ROOT/venv-current")" == "venv/$APPLICATION_SHA" ]] || fail "prepared Control venv link mismatch"
  [[ "$(stat -c '%a:%U:%G' "$CONFIG_ROOT")" == "750:root:$RUNTIME_GROUP" ]] || fail "prepared Control configuration metadata mismatch"
  [[ "$(stat -c '%a:%U:%G' "$CONFIG_ROOT/runtime.env")" == "640:root:$RUNTIME_GROUP" ]] || fail "prepared Control runtime environment metadata mismatch"
  [[ "$(stat -c '%a:%U:%G' "$CONFIG_ROOT/core-identities.json")" == "600:$RUNTIME_USER:$RUNTIME_GROUP" ]] || fail "prepared Control identities metadata mismatch"
  /usr/bin/cmp --silent "$CONFIG_ROOT/runtime.env" "$control_target/config/adult-publishing/staging-s0-commercial/runtime.env.example" || fail "prepared Control runtime environment drift"
  /usr/bin/cmp --silent "$CONFIG_ROOT/core-identities.json" "$control_target/config/adult-publishing/staging-s0-commercial/core-identities.synthetic.json" || fail "prepared Control identities drift"
  ...
  verify_schema_acceptance "$app_target"
  /bin/systemctl is-active --quiet tu1nz-adult-publishing-s1.service || fail "STAGING-S1 is not active"
  ! /bin/systemctl is-active --quiet tu1nz_encrypted_backup.service || fail "backup service is active during prepared-Control advance"
  /bin/systemctl is-active --quiet tu1nz_encrypted_backup.timer || fail "backup timer is not active before prepared-Control advance"
  ! /bin/systemctl is-active --quiet "$UNIT" || fail "commercial unit is active"
  echo "M4_23_PREPARED_CONTROL_BOUNDARY_OK"
}

advance_prepared_control() {
  local control_target="$RELEASE_ROOT/control/$CONTROL_SHA"
  local stage_root="/opt/tu1nz_repos/.m4-23-commercial-s0-control-stage-$CONTROL_SHA"
  local control_stage="$stage_root/control-$CONTROL_SHA"
  local next_link="$RELEASE_ROOT/control-current.m4-23-$CONTROL_SHA"
  ...
  /bin/systemctl stop tu1nz_encrypted_backup.timer
  BACKUP_TIMER_PAUSED=1
  /usr/bin/install -o root -g root -m 0600 "$INSTALLED_BACKUP" "$backup_evidence"
  /usr/bin/install -o root -g root -m 0755 "$CONTROL_REPOSITORY/scripts/tu1nz_encrypted_backup.sh" "$INSTALLED_BACKUP"
  [[ "$(sha256 "$INSTALLED_BACKUP")" == "$(sha256 "$CONTROL_REPOSITORY/scripts/tu1nz_encrypted_backup.sh")" ]] || fail "commercial backup script advance drift"
  ...
  /bin/systemctl is-active --quiet tu1nz_encrypted_backup.timer || fail "backup timer did not resume"
  trap - EXIT
  echo "M4_23_STOPPED_CANDIDATE_PREPARED_OK advanced_prepared_control"
}
...
case "$MODE" in
  ...
  advance-prepared-control) advance_prepared_control ;;
